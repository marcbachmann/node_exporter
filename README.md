...
Be aware that any non-root mount points you want to monitor will need bind-mounted into the container.

If you start container for host monitoring, specify `path.rootfs` argument.
This argument must match path in bind-mount of host root. The node\_exporter will use `path.rootfs` as prefix to filter entries in ${path.procfs}/mounts and to cleanup it from `mountpoint` label.

Also you need to use `bind-propagation=rslave` option for bind-mount host rootfs to propagate host mounts changes to container (option available since 17.05.0).
